...
function RegistrationForm() {
    ...
    const dateForUserName = () => {
        ...
        const userMonth = thisMonth < 10 ? "0" + thisMonth : thisMonth;
        return `${userMonth}${twoDigitYear}`;
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        const dbUserName = firstName.charAt(0) + lastName.toLowerCase() + dateForUserName();
        const accountCreatedBy = "admin"; // or however you're determining this
    
        try {
            const response = await axios.post(`http://localhost:8080/hey/create?accountCreatedBy=${accountCreatedBy}`, {
                firstName,
                lastName,
                address,
                birthMonth,
